...
describe("run", () => {
    it("should run all input assertions and return a result object with success status and messages", async () => {
        ...
        assert(result, false, 17, 14, "Test1.1: ✅ The expected (5) is EQUAL to actual (5)\n" +
            ...
            "Test10.1: ✅ 2 is LESS THAN or EQUAL 2\n" +
            "Test10.2: ❌ 10 is NOT LESS THAN or EQUAL 2\n" +
            "Test10.3: ✅ 0 is LESS THAN or EQUAL 2\n" +
    ...
    it("set fail fast true. should break with first error.", async () => {
        ...
        assert(result, false, 2, 1, "Test1.1: ✅ 'true' is TRUE\n" +
    ...
    it("all asserts are valid", async () => {
        ...
        assert(result, true, 5, 0, "Test1.1: ✅ 'true' is TRUE\n" +
            "Test1.2: ✅ 'true' is TRUE\n" +
            "Test2.1: ✅ '' is FALSE\n" +
...
function assert(result: IControllerOutput, expectedSuccess: boolean, expectedSuccessMessages: number, expectedFailureMessages: number, expectedMessageStr: string) {
    expect(result.success).toBe(expectedSuccess);

    expect(result.messages.length).toBe(expectedSuccessMessages + expectedFailureMessages);

    expect(result.messages.filter(message => message.type === 'error').length).toBe(expectedFailureMessages);
    expect(result.messages.filter(message => message.type === 'success').length).toBe(expectedSuccessMessages);
